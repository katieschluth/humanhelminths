...
# The helminth data
raw <- read_csv('C:/Users/cjcar/Documents/GitHub/humanhelminths/FinalDataFigs/WormMapsDB-Grid view.csv')
...
raw %>% mutate(Countries = str_replace_all(as.character(Countries), 
                                                  pattern=' and ',
                                                  replacement=', ')) %>% 
  separate_rows(Countries, sep=',') %>%
  left_join(regions %>% select(ISO, NAME_ENGLI) %>% dplyr::rename(Countries = NAME_ENGLI) %>% unique()) -> 
  raw.split
...
raw.split %>% 
  mutate(Countries.Coded = trimws(as.character(Countries))) %>% 
  ...
  mutate(Countries.Coded = recode(Countries.Coded, !!!ittybittyboys)) %>%
  mutate(Countries.Coded = recode(Countries.Coded, !!!bigboys)) %>%
  select(-c(Countries)) %>% distinct() -> region.clean 
